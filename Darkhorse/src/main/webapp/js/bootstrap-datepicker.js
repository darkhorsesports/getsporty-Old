...
!function($) {
	...
	var Datepicker = function(element, options) {
		...
		this.language = options.language || this.element.data('date-language')
				|| "en";
		this.language = this.language in dates ? this.language : this.language
				.split('-')[0]; // Check if "de-DE" style date is available, if
								// not language should fallback to 2 letter code
								// eg "de"
		this.language = this.language in dates ? this.language : "en";
		this.isRTL = dates[this.language].rtl || false;
		this.format = DPGlobal.parseFormat(options.format
				|| this.element.data('date-format')
				|| dates[this.language].format || 'mm/dd/yyyy');
		this.isInline = false;
		...
		this.picker = $(DPGlobal.template).appendTo(
...
Datepicker.prototype = {
		...
		update : function() {
			...
			if (arguments
				...
				date = this.isInput ? this.element.val() : this.element
						.data('date')
						|| this.element.find('input').val();
			}
		...
		fill : function() {
			var d = new Date(this.viewDate), year = d.getUTCFullYear(), month = d
					...
					.getUTCMonth()
					: Infinity, currentDate = this.date && this.date.valueOf(), today = new Date();
			...
			while (prevMonth.valueOf() < nextMonth) {
				if (prevMonth.getUTCDay() == this.weekStart) {
					...
					if (this.calendarWeeks) {
						...
						var a = new Date(prevMonth.getUTCFullYear(), prevMonth
								.getUTCMonth(),
								prevMonth.getUTCDate()
										- prevMonth.getDay()
										+ 10
										- (this.weekStart
												&& this.weekStart % 7 < 5 && 7)), b = new Date(
								a.getFullYear(), 0, 4), calWeek = ~~((a - b) / 864e5 / 7 + 1.5);
